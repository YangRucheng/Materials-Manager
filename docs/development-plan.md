...
| 角色 | 主要权限 |
| --- | --- |
| 超级管理员 | 用户和基础数据维护、全部业务操作，以及已归档申购计划查询 |
...
3. 系统实时汇总该物资近 6 个自然月内未被冲销的实际出库量，并将其作为建议申购数量。
...
发起补库不会创建申购记录。补库表单默认使用当天作为需求日期，并预填最近一条申购计划的申购负责人；用户可修改需求日期、计划数量、实际需求人和申购负责人。确认后系统仅新增申购计划，计划编码后可单条或批量转入正式申购记录。

已归档申购计划仅超级管理员可查询和打开。其他角色的列表固定查询正常计划，前端不展示“已归档”或“全部状态”筛选项，后端对直接查询归档状态或归档详情返回 403。
...
suggested_purchase_qty = 近 6 个自然月内未被冲销的 OUTBOUND 流水数量之和
```

建议数量在库存列表查询和确认发起补库时分别实时计算。即使近 6 个月没有出库记录，低库存物资仍允许由用户手工填写计划数量发起补库。
...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/stock-materials` | 按名称、型号查询物资档案 |
| POST | `/stock-materials` | 创建物资 |
| GET | `/stock-materials/{id}` | 详情，含图片、余额和补库策略 |
| PATCH | `/stock-materials/{id}` | 修改基础信息，不允许改库存 |
| PUT | `/stock-materials/{id}/replenishment-policy` | 设置最低库存和预警开关 |
| GET | `/inventory/balances` | 库存查询，支持名称、型号、库存范围、低库存筛选 |
| GET | `/inventory/replenishment-defaults` | 获取补库默认负责人和当天需求日期 |
| GET | `/inventory/low-stock` | 低库存与近 6 个月消耗建议列表 |
...
- 未编码申购计划数量
- 待处理请购、部分到货请购
- 低库存提醒仅展示前 5 条只读数据，完整列表通过“查看全部”进入库存查询

#### 库存查询

表格列：物资名称、型号规格、单位、当前库存、预警状态、更新时间、操作。

行操作：查看详情、入库、出库、发起补库。建议申购数量实时取近 6 个自然月的有效出库消耗；发起补库成功后跳转到新建的申购计划详情，不跳转申购记录。
...
5. 低库存页能显示当前库存、最低库存和近 6 个自然月实时出库消耗形成的建议申购数量。
